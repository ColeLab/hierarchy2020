...
h5f = h5py.File('../../resultdata/actflow/activations_actflow_analysis.h5','a')
try: 
...
#######################################################################################################
#### Run actflow for group-level
fc = np.mean(restfc,axis=2)
np.fill_diagonal(fc,0)
act = np.mean(task_activity,axis=2)
actflow = np.matmul(act.T,fc).T
#######################################################################################################
...
#######################################################################################################
#### Save out activations and actflow predictions
h5f = h5py.File('../../resultdata/actflow/activations_actflow_analysis_group.h5','a')
try: 
    h5f.create_dataset('real',data=act)
    h5f.create_dataset('actflow',data=actflow)
except:
    del h5f['real'], h5f['actflow']
    h5f.create_dataset('real',data=act)
    h5f.create_dataset('actflow',data=actflow)
