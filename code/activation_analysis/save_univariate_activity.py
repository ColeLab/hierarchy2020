...
#######################################################################################################
#### Load data
taskAct = np.zeros((nParcels,nTasks,len(subjNums)))
scount = 0
for subj in subjNums:
    if scount%25==0: print('Loading in data for subject', scount+1, '/', len(subjNums))
    tmp = []
    for task in taskNames:
        tmp.extend(tools.loadTaskActivityParcels(subj,task, model=model).T)
...
subj_act = np.mean(np.abs(taskAct),axis=1)

np.savetxt('../../resultdata/activity/activity_canonicalHRF.csv',t_act,delimiter=',')
np.savetxt('../../resultdata/activity/activity_canonicalHRF_allSubjs.csv',subj_act,delimiter=',')
