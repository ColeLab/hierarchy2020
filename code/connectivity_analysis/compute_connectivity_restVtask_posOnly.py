...
for subj in subjNums:
    ...
    taskTS[:,:,scount] = tools.loadTaskResiduals(subj, model=model, taskModel=taskModel, zscore=zscore) # We make this FIR since we only want to use FIR regressed task residuals for FC analysis
...
taskFC = np.zeros((nParcels,nParcels,len(subjNums)))
restFC = np.zeros((nParcels,nParcels,len(subjNums)))
#
taskGBC = np.zeros((nParcels,len(subjNums)))
restGBC = np.zeros((nParcels,len(subjNums)))

taskGVC = np.zeros((nParcels,len(subjNums)))
for s in range(len(subjNums)):
    if s%25==0: print('Subject ' + str(s))

    taskFC[:,:,s] = np.corrcoef(taskTS[:,np.where(task_timing)[0],s])
    restFC[:,:,s] = np.corrcoef(restTS[:,np.where(task_timing)[0],s])
    np.fill_diagonal(taskFC[:,:,s],0)
    np.fill_diagonal(restFC[:,:,s],0)
    
    ## Apply threshold
    taskFC[:,:,s] = np.multiply(taskFC[:,:,s],taskFC[:,:,s]>0)
    restFC[:,:,s] = np.multiply(restFC[:,:,s],restFC[:,:,s]>0)

    taskGBC[:,s] = np.mean(np.arctanh(taskFC[:,:,s]),axis=1)
    restGBC[:,s] = np.mean(np.arctanh(restFC[:,:,s]),axis=1)



taskVrest_gbc_diff = taskGBC-restGBC

#######################################################################################################
np.savetxt('../../resultdata/connectivity/conn_taskVrest_gbc_' + taskModel + '_posOnly.csv',taskVrest_gbc_diff,delimiter=',')
#######################################################################################################
